...
prepare() {
	...
	sed -i '387s/#.*//' arch/arm/mm/proc-v7.S
	sed -i '117s/#.*//' arch/arm/boot/compressed/head.S
	sed -i 's/,#alloc/,"a"/g' arch/arm/boot/compressed/Makefile
	sed -i 's/#alloc/"a"/g' arch/arm/boot/compressed/Makefile
	if [ -f arch/arm/boot/compressed/piggy.gzip.S ]; then
		sed -i 's/#alloc/"a"/g' arch/arm/boot/compressed/piggy.gzip.S
	fi
}

build() {
	unset LDFLAGS
	_myflags="-std=gnu89 -fpermissive -Wno-int-conversion -Wno-incompatible-pointer-types -Wno-implicit-function-declaration -Wno-format-security -Wno-return-type"
	
	export HOSTCFLAGS="$_myflags"
	export KCFLAGS="$_myflags"
